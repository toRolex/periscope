...
(0, node_test_1.test)('buildRequest 构造 anthropic v1/messages 请求', () => {
    const req = anthropic_js_1.anthropicAdapter.buildRequest({
...
(0, node_test_1.test)('extractText 提取 content 中 text 块的拼接文本', () => {
    ...
    assert.equal(anthropic_js_1.anthropicAdapter.extractText(raw), '左边是山，右边是河');
});
(0, node_test_1.test)('extractText 忽略非 text 块（如 tool_use）', () => {
    ...
    assert.equal(anthropic_js_1.anthropicAdapter.extractText(raw), '画面里有只猫');
});
(0, node_test_1.test)('extractText 对非 JSON 响应透传原始文本', () => {
    assert.equal(anthropic_js_1.anthropicAdapter.extractText('not-json-at-all'), 'not-json-at-all');
});
(0, node_test_1.test)('extractText 对缺少 content 的响应透传原始文本', () => {
    const raw = '{"error":{"message":"bad request"}}';
    assert.equal(anthropic_js_1.anthropicAdapter.extractText(raw), raw);
});
