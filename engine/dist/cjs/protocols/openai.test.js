...
(0, node_test_1.test)('buildRequest 构造 openai chat/completions 请求', () => {
    const req = openai_js_1.openaiAdapter.buildRequest({
...
(0, node_test_1.test)('extractText 提取字符串 content', () => {
    const text = openai_js_1.openaiAdapter.extractText('{"choices":[{"message":{"content":"画面里有一只猫"}}]}');
...
(0, node_test_1.test)('extractText 拼接数组 content 中的文本片段', () => {
    ...
    assert.equal(openai_js_1.openaiAdapter.extractText(raw), '左边是山，右边是河');
});
(0, node_test_1.test)('extractText 对非 JSON 响应透传原始文本', () => {
    assert.equal(openai_js_1.openaiAdapter.extractText('not-json-at-all'), 'not-json-at-all');
});
(0, node_test_1.test)('extractText 对缺少 content 的响应透传原始文本', () => {
    const raw = '{"error":{"message":"bad request"}}';
    assert.equal(openai_js_1.openaiAdapter.extractText(raw), raw);
});
