...
function fakeSettings(base: Record<string, unknown>): PeriscopeSettingsPort & { user: Record<string, unknown> } {
...
test('Seam 3 端到端冒烟：settings 写入 → 插件读配置 → describe 视觉端点 → mock 返回', async (t) => {
  ...
  if (!effResult.ok) return;
  const effective = effResult.value as {
  ...
  const body = req.jsonBody as { model: string; messages: { content: unknown[] }[] };
  assert.equal(body.model, 'settings-model', 'describe 请求体应使用归并生效的 model');
  assert.equal(
    (body.messages[0].content[1] as { image_url: { url: string } }).image_url.url,
...
test('Seam 3：完全未配置 → 生效值未就绪，describe 返回引导占位符、不请求视觉端点', async (t) => {
  ...
  assert.equal(effResult.ok, true);
  if (!effResult.ok) return;
  const effective = effResult.value as { configured: boolean };
