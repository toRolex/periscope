...
test('describe 注入空白模板 config（baseUrl/model 为空串）时抛出可操作报错并提示运行 init', async () => {
  ...
  await assert.rejects(
    describe({ bytes: PNG_BYTES }, { config }),
    /协议 openai 未配置 baseUrl\/model，请运行 init/,
  );
});
...
test('describe 空端点 config 时不发起任何请求（transport 不被调用）', async () => {
  ...
  await assert.rejects(
    describe({ bytes: PNG_BYTES }, { config, transport: fakeTransport }),
    /未配置 baseUrl\/model/,
  );
...
test('describe 尊重调用方 mimeType：image/jpeg 字节 → data URL 前缀为 image/jpeg', async (t) => {
  ...
  const body = server.requests[0].jsonBody as any;
  assert.ok(
    (body.messages[0].content[1].image_url.url as string).startsWith(
...
test('describe 通过 mock 端点发送 anthropic v1/messages 请求并提取文本', async (t) => {
  ...
  assert.equal(
    ...
    PNG_1PX_BASE64,
    'anthropic image data 应为图片字节的精确 base64',
  );
});
