...
function bench(
  ...
  pingResult: unknown = { ok: true, value: { ok: true, message: '端点可达（HTTP 200）' } },
): Bench {
  const calls: RpcCall[] = [];
  ...
  const rpc = {
    call: async (channel: string, endpoint: string, payload: unknown): Promise<unknown> => {
      ...
      if (endpoint === 'ping') return pingResult;
      return updateResult;
    },
  };
  const ctx = {
...
test('表单只预填 user 层：cordis.yml 有值但 user 层缺省时不回填 cordis 值（保存不冻结 cordis 源字段）', async () => {
  ...
  const patch = (updateCall.payload as { patch: Record<string, string> }).patch;
...
test('卡片渲染「测试连接」按钮（data-action=ping），点击前不显示探测结果', async () => {
  ...
  assert.throws(
    () => renderer.root.findByProps({ 'data-ping-result': 'reachable' }),
    undefined,
    '未点击时不应显示探测结果',
  );
});
