...
import { DEFAULT_VISION_API_KEY_ENV, DEFAULT_VISION_PROTOCOL, VISION_ENV, mergeVisionInputs, normalizeSettingsSection, resolveVisionConfig, resolveVisionConfigWithSettings, } from './vision-config.js';
/**
...
test('normalizeSettingsSection：保留非空白四字段，空白/空白串视为未配置丢弃', () => {
    ...
    assert.deepEqual(normalizeSettingsSection({ protocol: '   ', baseUrl: '', model: undefined }), {}, '空白/缺省字段全部丢弃');
});
test('normalizeSettingsSection：非法 protocol 原样透传（校验归 resolveVisionConfig 裁决）', () => {
    assert.deepEqual(normalizeSettingsSection({ protocol: 'not-a-protocol' }), { protocol: 'not-a-protocol' });
});
// ── #33 settings 与 cordis.yml 合并（mergeVisionInputs） ─────────────────────
test('mergeVisionInputs：settings 逐字段优先于 cordis；settings 缺省时回落 cordis', () => {
    const merged = mergeVisionInputs({ protocol: 'openai', baseUrl: 'https://yml.example.com/v1', model: 'yml-model' }, { protocol: 'anthropic', model: 'settings-model' });
...
test('mergeVisionInputs：空白 settings 视为未配置，回落到 cordis', () => {
    const merged = mergeVisionInputs({ baseUrl: 'https://yml.example.com/v1', model: 'yml-model' }, { baseUrl: '   ', model: '' });
...
test('resolveVisionConfigWithSettings：settings/cordis 均缺 baseUrl → env fallback', () => {
    const cfg = resolveVisionConfigWithSettings({ model: 'yml-model' }, { model: '' }, { [VISION_ENV.baseUrl]: 'https://env.example.com/v1' });
...
test('resolveVisionConfigWithSettings：settings 缺省 → 等价于原 cordis + env 双来源', () => {
    const withSettings = resolveVisionConfigWithSettings({ baseUrl: 'https://yml.example.com/v1' }, undefined, { [VISION_ENV.model]: 'env-model' });
...
test('apiKey 仅从 env 读取：settings/cordis 均无法注入字面 key（归一化输入无 apiKey 字段）', () => {
    const maliciousSettings = { apiKey: 'sk-from-settings' };
...
test('resolveVisionConfigWithSettings：非法 settings protocol 回落默认 openai（与 cordis 同契约）', () => {
    const cfg = resolveVisionConfigWithSettings({}, { protocol: 'not-a-protocol' }, {});
    assert.equal(cfg.protocol, DEFAULT_VISION_PROTOCOL);
});
