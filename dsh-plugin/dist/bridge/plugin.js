...
export function apply(ctx, config) {
    ...
    const resolveVision = () => resolveVisionConfigWithSettings(config, currentSettings(), process.env);
...
function registerPeriscopeRpc(ctx, resolveVision) {
    ctx.inject(['connection'], (connectionCtx) => {
        ...
        const port = {
            ...
            update: async (patch, expectedRevision) => {
                const provider = settings();
                if (provider === undefined)
                    throw new Error('settings 服务不可用：periscope 命名空间未注册');
                await provider.update(NS, patch, expectedRevision);
            },
        };
        connectionCtx.connection.rpc.handle('/periscope', makePeriscopeRpcHandler(port, NS, {
            probe: {
