...
exports.DEFAULT_VISION_PROTOCOL = exports.DEFAULT_VISION_API_KEY_ENV = exports.VISION_ENV = void 0;
exports.resolveVisionConfig = resolveVisionConfig;
/**
...
exports.DEFAULT_VISION_API_KEY_ENV = 'PERISCOPE_API_KEY';
/** protocol 的缺省值：仅指请求形状（openai 兼容协议），不绑定任何服务商。 */
exports.DEFAULT_VISION_PROTOCOL = 'openai';
 ...
 */
function resolveVisionConfig(input = {}, env = {}) {
    const protocolRaw = firstPresent(input.protocol, env[exports.VISION_ENV.protocol]);
    const protocol = VALID_PROTOCOLS.includes(protocolRaw)
        ? protocolRaw
        : exports.DEFAULT_VISION_PROTOCOL;
    const baseUrl = firstPresent(input.baseUrl, env[exports.VISION_ENV.baseUrl]) ?? '';
    const model = firstPresent(input.model, env[exports.VISION_ENV.model]) ?? '';
    const apiKeyEnv = firstPresent(input.apiKeyEnv) ?? exports.DEFAULT_VISION_API_KEY_ENV;
