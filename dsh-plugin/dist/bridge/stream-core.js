...
export function visionEndpointReady(vision) {
 ...
 */
export function buildDescribeImage(vision, describeFn, options = {}) {
 ...
 */
export async function translateMessages(messages, deps) {
    const records = [];
    let anyImage = false;
    const out = [];
    for (const message of messages) {
        // translateContent 不改写入参 content，故 readonly → mutable 的窄化转换是安全的。
        const result = await translateContent(message.content, deps);
 ...
 */
export function makeImageDescribedSink(deps) {
    return {
...
export function emitImageDescribed(sink, sessionId, records) {
