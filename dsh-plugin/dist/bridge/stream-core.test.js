...
import { DESCRIBE_TIMEOUT_MS, ENDPOINT_NOT_CONFIGURED_GUIDANCE, buildDescribeImage, emitImageDescribed, makeImageDescribedSink, toPeriscopeConfig, translateMessages, visionEndpointReady, } from './stream-core.js';
/**
...
test('toPeriscopeConfig：激活 anthropic 时端点进 anthropic 段', () => {
    const cfg = toPeriscopeConfig(readyVision({ protocol: 'anthropic', baseUrl: 'https://anth.example.com', model: 'claude-vision' }));
...
test('buildDescribeImage：超时 → reject（由 translateContent 降级为占位符，绝不悬挂会话）', async () => {
    const describeImage = buildDescribeImage(readyVision(), () => new Promise(() => { }), // 永不解决，模拟悬挂端点
    { timeoutMs: 20 });
