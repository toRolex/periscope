...
import { makeTempDir, PNG_1PX_BASE64, readyEndpoint, writeConfigFile, } from '../testing/fixtures.js';
/**
...
test('describe 通过 mock 端点发送 openai 协议请求并提取文本（图片字节 → data URL 精确回传）', async (t) => {
    const server = await createMockServer({
        defaultBody: JSON.stringify({ choices: [{ message: { content: '图片里有一座山' } }] }),
    });
    t.after(() => server.close());
    const config = writeConfigFile(makeTempDir(), {
        apiKey: 'sk-core',
        openai: readyEndpoint(server.baseUrl),
    }).config;
    const text = await describe({ bytes: PNG_BYTES }, { config });
...
test('describe 注入空白模板 config（baseUrl/model 为空串）时抛出可操作报错并提示运行 init', async () => {
    const config = writeConfigFile(makeTempDir()).config; // 空白模板：三协议 baseUrl/model 均为空串
    await assert.rejects(describe({ bytes: PNG_BYTES }, { config }), /协议 openai 未配置 baseUrl\/model，请运行 init/);
});
test('describe 空端点 config 时不发起任何请求（transport 不被调用）', async () => {
    let calls = 0;
    const fakeTransport = {
        async post() {
            calls += 1;
    ...
    const config = writeConfigFile(makeTempDir()).config; // 空白模板
    await assert.rejects(describe({ bytes: PNG_BYTES }, { config, transport: fakeTransport }), /未配置 baseUrl\/model/);
    assert.equal(calls, 0, '空端点不应发出请求');
});
test('describe 只缺 baseUrl 时同样报错并提示运行 init', async () => {
    const config = writeConfigFile(makeTempDir(), {
...
test('describe 透传 intent 到 text 部分', async (t) => {
    ...
    await describe({ bytes: PNG_BYTES, intent: '用中文描述颜色' }, { config });
    const body = server.requests[0].jsonBody;
    assert.equal(body.messages[0].content[0].text, '用中文描述颜色');
});
test('describe intent 为 ocr 时请求体使用 OCR 模板 prompt', async (t) => {
    const server = await createMockServer();
    t.after(() => server.close());
    const config = writeConfigFile(makeTempDir(), {
        openai: readyEndpoint(server.baseUrl),
    }).config;
    await describe({ bytes: PNG_BYTES, intent: 'ocr' }, { config });
    const body = server.requests[0].jsonBody;
    assert.equal(body.messages[0].content[0].text, TASK_TEMPLATES.ocr);
});
test('describe intent 为 table 时请求体使用 table 模板 prompt', async (t) => {
    const server = await createMockServer();
    t.after(() => server.close());
    const config = writeConfigFile(makeTempDir(), {
        openai: readyEndpoint(server.baseUrl),
    }).config;
    await describe({ bytes: PNG_BYTES, intent: 'table' }, { config });
    const body = server.requests[0].jsonBody;
    assert.equal(body.messages[0].content[0].text, TASK_TEMPLATES.table);
});
test('describe intent 为 chart 时请求体使用 chart 模板 prompt', async (t) => {
    const server = await createMockServer();
    t.after(() => server.close());
    const config = writeConfigFile(makeTempDir(), {
        openai: readyEndpoint(server.baseUrl),
    }).config;
    await describe({ bytes: PNG_BYTES, intent: 'chart' }, { config });
    const body = server.requests[0].jsonBody;
...
test('describe 自定义 intent 原样透传：模板名保留字之外的文本不被模板化', async (t) => {
    ...
    await describe({ bytes: PNG_BYTES, intent: '把图片中的表格整理成要点' }, { config });
    const body = server.requests[0].jsonBody;
...
test('describe intent 为 ocr 在 anthropic 协议请求体使用 OCR 模板 prompt', async (t) => {
    const server = await createMockServer();
    t.after(() => server.close());
    const config = writeConfigFile(makeTempDir(), {
        protocol: 'anthropic',
...
test('describe intent 为 table 在 responses 协议请求体使用 table 模板 prompt', async (t) => {
    const server = await createMockServer();
    t.after(() => server.close());
    const config = writeConfigFile(makeTempDir(), {
        protocol: 'responses',
...
test('describe 端点返回非 2xx 时抛错', async (t) => {
    ...
    await assert.rejects(describe({ bytes: PNG_BYTES }, { config }), /HTTP 401/);
});
...
test('describe 尊重调用方 mimeType：image/jpeg 字节 → data URL 前缀为 image/jpeg', async (t) => {
    const server = await createMockServer();
    t.after(() => server.close());
    const config = writeConfigFile(makeTempDir(), {
        openai: readyEndpoint(server.baseUrl),
    }).config;
    await describe({ bytes: PNG_BYTES, mimeType: 'image/jpeg' }, { config });
    const body = server.requests[0].jsonBody;
...
test('describe 通过 mock 端点发送 anthropic v1/messages 请求并提取文本', async (t) => {
    ...
    assert.equal(body.messages[0].content[1].source.data, PNG_1PX_BASE64, 'anthropic image data 应为图片字节的精确 base64');
});
test('describe 通过 mock 端点发送 responses v1/responses 请求并提取文本', async (t) => {
    ...
    assert.equal(body.input[0].content[1].image_url, `data:image/png;base64,${PNG_1PX_BASE64}`);
});
...
test('describe anthropic 端点返回非 2xx 时抛错', async (t) => {
    ...
    await assert.rejects(describe({ bytes: PNG_BYTES }, { config }), /HTTP 429/);
});
test('describeMany 并行请求多图并按输入顺序聚合；未传 source 时回退为「图片 N」', async () => {
...
test('describeMany 回显调用方 source 标识（供桥接层定位图片 / attachment）', async () => {
    const config = writeConfigFile(makeTempDir(), { openai: readyEndpoint('https://example.com') }).config;
    const fakeTransport = {
        async post() {
            return {
...
test('describe 未注入配置时走 loadConfig：环境变量优先于文件 apiKey', async (t) => {
    ...
    try {
        const text = await describe({ bytes: PNG_BYTES });
        assert.equal(text, 'mock 默认描述');
