...
test('空白模板：anthropic/responses 的 baseUrl 与 model 均为空串，不绑定任何服务商', () => {
    assert.equal(DEFAULT_CONFIG.anthropic.baseUrl, '');
    assert.equal(DEFAULT_CONFIG.anthropic.model, '');
    assert.equal(DEFAULT_CONFIG.responses.baseUrl, '');
    assert.equal(DEFAULT_CONFIG.responses.model, '');
});
test('配置文件不包含任何具体服务商默认值（无 DashScope / Anthropic / OpenAI 端点）', () => {
    const raw = JSON.stringify(DEFAULT_CONFIG);
...
test('默认配置路径为 HOME/.config/periscope/config.json，且可被 PERISCOPE_CONFIG 覆盖', () => {
    ...
    withEnv({ PERISCOPE_CONFIG: undefined, HOME: dir }, () => {
        assert.equal(defaultConfigPath(), path.join(dir, '.config', 'periscope', 'config.json'));
    });
...
test('configPathForEnv：PERISCOPE_CONFIG 优先，否则 HOME 派生', () => {
    const dir = makeTempDir();
    assert.equal(configPathForEnv({ HOME: dir }), path.join(dir, '.config', 'periscope', 'config.json'));
    const override = path.join(dir, 'custom.json');
    assert.equal(configPathForEnv({ HOME: dir, PERISCOPE_CONFIG: override }), override);
});
test('configPathForEnv：HOME 缺省时用 os.homedir() 兜底', () => {
    const dir = makeTempDir();
    withEnv({ HOME: dir }, () => {
        assert.equal(configPathForEnv({}), path.join(dir, '.config', 'periscope', 'config.json'));
    });
});
test('PERISCOPE_API_KEY 环境变量优先于配置文件中的 apiKey', () => {
    const dir = makeTempDir();
    const configPath = path.join(dir, 'config.json');
    fs.writeFileSync(configPath, JSON.stringify({ ...DEFAULT_CONFIG, apiKey: 'file-key' }));
...
test('配置文件只改 openai.baseUrl：openai.model 保留空白模板、其他协议段空白完整', () => {
    const dir = makeTempDir();
    const configPath = path.join(dir, 'config.json');
...
test('DEFAULT_CONFIG.protocol 为 Protocol 联合类型（openai/anthropic/responses）', () => {
    const p = DEFAULT_CONFIG.protocol;
