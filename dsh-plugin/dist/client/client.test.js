...
test('已配置（configured:true）时不显示未配置引导', async () => {
    ...
    const { registered } = bench(EMPTY_READ, { ok: true, value: null }, effective);
    const renderer = await renderCard(registered);
    assert.throws(() => renderer.root.findByProps({ 'data-guidance': true }), undefined, '已配置不应显示未配置引导');
});
...
test('describeEffective 失败 → 不渲染生效区，表单仍可用', async () => {
    const { registered } = bench(EMPTY_READ, { ok: true, value: null }, { ok: false, error: { code: 'settings-rejected', message: 'boom', details: { ns: 'periscope' } } });
    const renderer = await renderCard(registered);
